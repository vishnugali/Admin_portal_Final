...
export default function JobOpenings() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    const dbPayload = {
      ...
      is_time: formType === 'Full-time'
    };

    try {
      if (editingJob) {
        const { error } = await supabase.from('job_openings').update(dbPayload).eq('id', editingJob.id);
        if (error) throw error;
        alert('Job posting updated successfully!');
      } else {
        const colors = [{ bg: '#eff6ff', text: '#1d4ed8' }, { bg: '#fdf2f8', text: '#be185d' }, { bg: '#f0fdf4', text: '#15803d' }];
        const randomColor = colors[Math.floor(Math.random() * colors.length)];
        const newRecordPayload = { ...dbPayload, color: randomColor.bg, text_color: randomColor.text };
        const { error } = await supabase.from('job_openings').insert([newRecordPayload]);
        if (error) throw error;
            ...
            {jobs.map((job) => (
              <div key={job.id} style={rowStyle}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                  <div style={{ ...avatarStyle, backgroundColor: job.color || '#eff6ff', color: job.text_color || '#1d4ed8' }}>
        ...
        <div style={panelWrapperStyle}>
          <div style={innerPanelContainerStyle}>
            ...
            <div style={{ marginTop: '24px', marginBottom: '8px' }}>
              <h2 style={panelTitleStyle}>{editingJob ? 'Update' : 'Create'} <span style={{ color: '#0024DE' }}>Job Opening</span></h2>
            ...
            <form onSubmit={handleFormSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginTop: '28px' }}>
              <div style={formGridRowStyle}>
                <div style={formGroupStyle}>
                  ...
                  <input type="text" required placeholder="e.g. Research Scientist" style={inputStyle} value={formTitle} onChange={(e) => setFormTitle(e.target.value)} />
                </div>
                <div style={formGroupStyle}>
                  <label style={labelStyle}>DEPARTMENT</label>
                  <div style={{ position: 'relative' }}>
                    <select style={selectFieldStyle} value={formDept} onChange={(e) => setFormDept(e.target.value)}>
              ...
              {formDept === 'Other' && (
                <div style={formGroupStyle}>
                  <input type="text" required placeholder="Type custom department name..." style={inputStyle} value={formCustomDept} onChange={(e) => setFormCustomDept(e.target.value)} />
                </div>
              )}
              <div style={formGridRowStyle}>
                <div style={formGroupStyle}>
                  <label style={labelStyle}>LOCATION</label>
                  <input type="text" required placeholder="e.g. Bangalore, India" style={inputStyle} value={formLocation} onChange={(e) => setFormLocation(e.target.value)} />
                </div>
                <div style={formGroupStyle}>
                  <label style={labelStyle}>EXPERIENCE</label>
                  <input type="text" placeholder="e.g. 3+ years" style={inputStyle} value={formExperience} onChange={(e) => setFormExperience(e.target.value)} />
                </div>
              </div>
              <div style={formGridRowStyle}>
                <div style={formGroupStyle}>
                  ...
                  <div style={{ position: 'relative' }}>
                    <select style={selectFieldStyle} value={formType} onChange={(e) => setFormType(e.target.value)}>
                ...
                <div style={formGroupStyle}>
                  ...
                  <input type="text" placeholder="e.g. 12-18 LPA" style={inputStyle} value={formSalary} onChange={(e) => setFormSalary(e.target.value)} />
                </div>
              </div>
              <div style={formGroupStyle}>
                <label style={labelStyle}>DESCRIPTION</label>
                <textarea placeholder="Role and responsibilities..." style={textareaStyle} value={formDescription} onChange={(e) => setFormDescription(e.target.value)} />
              </div>
              <div style={formGroupStyle}>
                <label style={labelStyle}>KEY SKILLS</label>
                <input type="text" placeholder="Skill 1, Skill 2, Skill 3" style={inputStyle} value={formTags} onChange={(e) => setFormTags(e.target.value)} />
              </div>
...
// Fixed Styling Objects (CamelCase applied)
